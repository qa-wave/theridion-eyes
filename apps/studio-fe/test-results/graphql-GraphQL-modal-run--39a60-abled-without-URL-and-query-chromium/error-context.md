...
- Name: graphql.spec.ts >> GraphQL modal >> run button is disabled without URL and query
- Location: tests/e2e/graphql.spec.ts:56:3
...
Expected: disabled
Error: strict mode violation: getByRole('button', { name: 'Run' }) resolved to 2 elements:
    1) <button type="button" title="Run collection" class="rounded p-0.5 text-neutral-600 opacity-0 transition hover:bg-neutral-800 hover:text-emerald-400 group-hover:opacity-100">…</button> aka getByRole('button', { name: 'Run collection' })
    2) <button disabled type="button" class="bg-accent-gradient inline-flex items-center gap-1.5 rounded-md px-4 py-1.5 text-xs font-medium text-white shadow-glow-sm transition disabled:opacity-40 disabled:shadow-none">…</button> aka getByRole('button', { name: 'Run', exact: true })

Call log:
  - Expect "toBeDisabled" with timeout 5000ms
  - waiting for getByRole('button', { name: 'Run' })
